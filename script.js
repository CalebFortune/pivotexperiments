Parse.initialize(
  "yCVQ6n5s2B2UlAjGznIJy48ZGVqDqWvPkLDafztR",
  "onVLFcQX8q20jhSVlinFQS9194m5G3G5tc6Waxfi"
);
...
function populateProjectTypes() {
  ...
  userData.projectTypes.forEach((projectType) => {
    if (projectType) {
      // Check if the projectType is not an empty string
      const option = document.createElement("option");
...
function populatePersonas() {
  ...
  userData.personas.forEach((persona) => {
    if (persona) {
      // Check if the persona is not an empty string
      const option = document.createElement("option");
...
function collectUserData() {
  ...
  userData.projectTypes = Array.from(
    document.querySelectorAll(".projectType")
  ).map((el) => el.value);
  userData.personas = Array.from(document.querySelectorAll(".persona")).map(
    (el) => el.value
  );
}
...
function validateUserData() {
  ...
  switch (pageId) {
    ...
    case "keyFactorsPage":
      ...
      if (
        userData.projectTypes.some((pt) => !pt) ||
        userData.personas.some((p) => !p)
      ) {
        alert("Please fill in all fields on this page.");
        return false;
      }
...
async function tuneTitles(generatedTitles) {
  let data = {
              ...
              items: {
                ...
                properties: {
                  title: {
                    type: "string",
                    description:
                      "The value of the title field in the prompt message",
                  },
                  projectType: {
                    type: "string",
                    description:
                      "The value of the projectType field in the prompt message",
                  },
                  persona: {
                    type: "string",
                    description:
                      "The value of the persona field in the prompt message",
                  },
                },
                required: ["title", "projectType", "persona"],
              },
            },
          },
        },
  ...
  const headers = {
    Authorization: `Bearer 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`,
  ...
  try {
    ...
    let wantsToUseFunction = res.choices[0].finish_reason == "function_call";

    if (wantsToUseFunction) {
      if ((res.choices[0].message.function_call.name = "helloWorld")) {
...
async function submitData() {
  ...
  const directTitles = userData.ideas
    .filter((idea) => idea.type === "directTitle")
    .map((idea) => idea.title);
  const topicClusters = userData.ideas
    .filter((idea) => idea.type === "topicCluster")
    .map((idea) => idea.topic);
  ...
  Parse.Cloud.run("generateTitles", {
    ...
    .then(async (generatedTitles) => {
      const g = await tuneTitles(generatedTitles[0]);

      generatedTitles = g.addresses;
      // Map the directTitles to ensure they have the correct format
      const formattedDirectTitles = directTitles.map((titleObj) => ({
        title:
          titleObj.title && titleObj.title.title ? titleObj.title.title : "", // Check if titleObj.title and titleObj.title.title exist
      ...
      const formattedGeneratedTitles = generatedTitles
        .filter(
          (title) =>
            title &&
            typeof title === "object" &&
      ...
      console.log(formattedGeneratedTitles);
      // Merge the formattedDirectTitles with the formattedGeneratedTitles
      const allDirectTitles = formattedDirectTitles.concat(
        formattedGeneratedTitles
      );
...
function toggleIdeaTypeFields() {
  ...
  document.getElementById("directTitleFields").style.display =
    ideaType === "directTitle" ? "block" : "none";
  document.getElementById("topicClusterField").style.display =
    ideaType === "topicCluster" ? "block" : "none";
}
function populateSummary() {
  ...
  document.getElementById("summaryFrequencyType").textContent =
    userData.frequencyType;
  document.getElementById("summaryProjectTypes").textContent =
    userData.projectTypes.join(", ");
  document.getElementById("summaryPersonas").textContent =
    userData.personas.join(", ");
  document.getElementById("summaryTotalIdeas").textContent =
    userData.ideas.length;
...
async function organizeContentIdeas(ideas) {
  ...
  const industryKeywords = await fetchIndustryKeywordsFromServer(
    userData.industry
  );
  ...
  const validIdeas = ideas.filter(
    (idea) => idea.title && idea.projectType && idea.persona
  );
...
function getIdeaScore(idea, industryKeywords) {
  ...
  if (
    ...
    industryKeywords.some(
      (keyword) => typeof keyword === "string" && idea.title.includes(keyword)
    )
  ) {
...
function initializeCalendar() {
  ...
  if (calendarEl && !calendar) {
    // Check if calendar element exists and if calendar is already initialized
...
function assignDatesToIdeas(ideas) {
  ...
  const incrementDays =
    userData.frequencyType === "week"
      ? 7 / userData.frequency
      : 30 / userData.frequency;
...
document
  .getElementById("exportToCalendarButton")
...
async function fetchIndustryKeywordsFromServer() {
  try {
    const response = await Parse.Cloud.run("getIndustryKeywords", {
      industryName: userData.industry,
    });
    return response;
  } catch (error) {
...
document
  .getElementById("ideaInputNextButton")
...
document
  .getElementById("ideaType")
  .addEventListener("change", toggleIdeaTypeFields);
